...
const Logo = () => (
    <div className="flex items-center space-x-2">
        ...
        <span className="font-bold text-xl text-white">FigmaKit</span>
    </div>
);
    ...
    <div>
        ...
        <ul className="mt-4 space-y-3">
            {column.links.map(link => (
                <li key={link.name}>
                    <a href={link.href} className="text-base text-gray-400 hover:text-white">{link.name}</a>
                </li>
            ))}
        </ul>
    </div>
                ...
                <div className="xl:grid xl:grid-cols-3 xl:gap-8">
                    <div className="space-y-8 xl:col-span-1">
                        ...
                        <p className="text-gray-400 text-base">
                            The ultimate component library to build beautiful products, faster.
                        </p>
                    ...
                    <div className="mt-12 grid grid-cols-2 gap-8 xl:mt-0 xl:col-span-2">
                        <div className="md:grid md:grid-cols-2 md:gap-8 col-span-2">
                           <div className="grid grid-cols-2 md:grid-cols-3 gap-8">
                                {FOOTER_COLUMNS.map(col => <FooterColumnComponent key={col.title} column={col} />)}
                           </div>
                        </div>
                    </div>
                </div>
                <div className="mt-12 border-t border-gray-700 pt-8">
                    <p className="text-base text-gray-400 xl:text-center">&copy; {new Date().getFullYear()} FigmaKit. All rights reserved.</p>
                </div>
            </div>
        </footer>
    );
